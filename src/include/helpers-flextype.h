...
Vec dgdtheta(Mat L,Mat F, std::vector<Mat> M) {
...
Scalar logsumexp(Vec l) {
  // compute log(exp(pp1)*ww1 + ... + exp(ppd)*wwd)
  int n = l.size();
  if (n == 1) return l(0);
  
  std::sort(l.data(),l.data()+l.size(),std::greater<Scalar>());
  Vec S;
  S.setZero(n);
  S(0) = l(0);
...
Vec vector_sumexp(Mat vv,Vec w) {
  ...
  Vec out(m);
  out.setZero();
  Scalar themaxpos = -INFINITY,themaxneg = -INFINITY;
  Scalar pos = 0.,neg = 0.;
